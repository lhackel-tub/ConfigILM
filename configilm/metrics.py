...
def _get_binary_metrics(average: Optional[Literal["macro", "micro", "sample"]] = None, prefix: str = "",
                        exclude: Optional[list[str]] = None) -> MetricCollection:
    raise NotImplementedError("Binary classification metrics are not implemented yet.")


def _get_multiclass_metrics(num_classes: int, average: Optional[Literal["macro", "micro", "sample"]] = None,
                            prefix: str = "", exclude: Optional[list[str]] = None) -> MetricCollection:
    raise NotImplementedError("Multiclass classification metrics are not implemented yet.")


def _get_multilabel_metrics(num_labels: int, average: Optional[Literal["macro", "micro", "sample"]] = None,
                            prefix: str = "", exclude: Optional[list[str]] = None) -> MetricCollection:
    if exclude is None:
        exclude = []
    ...
    if average == "macro":
        ...
        ], prefix=prefix, postfix="_macro")
    elif average == "micro":
        return MetricCollection([
            metric(num_labels=num_labels, average="micro", task="multilabel") for metric in metric_list
        ], prefix=prefix, postfix="_micro")
    elif average == "sample":
        metric_list = _remove_metrics(_DEFAULT_METRICS, ["AUROC", "AveragePrecision"])
        return MetricCollection([
            metric(num_labels=num_labels, average="micro", task="multilabel", multidim_average='samplewise') for metric
            in metric_list
        ], prefix=prefix, postfix="_sample")
    elif average is None:
        return MetricCollection([
            metric(num_labels=num_labels, task="multilabel") for metric in metric_list
        ], prefix=prefix)
    else:
...
def get_classification_metric_collection(
    ...
    if prefix is None:
        prefix = ""
    if task == "binary":
        assert num_classes is None, "num_classes should not be provided for binary classification. This hints at an error in the code."
        assert num_labels is None, "num_labels should not be provided for binary classification. This hints at an error in the code."
        ...
        assert num_labels is None, "num_labels should not be provided for multiclass classification. This hints at an error in the code."
        return _get_multiclass_metrics(num_classes, average, prefix, exclude)
    elif task == "multilabel":
        assert num_classes is None, "num_classes should not be provided for multilabel classification. This hints at an error in the code."
