...
def test_ds_imgsize_pass(data_dir, img_size: Tuple[int, int, int]):
    ds = RSVQAHRDataSet(root_dir=data_dir, split="val", img_size=img_size, classes=94, seq_length=32)
...
def test_ds_classes_no_buckets(data_dir, classes: int):
    ds = RSVQAHRDataSet(root_dir=data_dir, classes=classes, split="train", quantize_answers=False)
    assert ds.classes == classes
    assert len(ds.selected_answers) == classes
...
def test_dm_dataloaders(data_dir, bs: int):
    dm = RSVQAHRDataModule(data_dir=data_dir, batch_size=bs, num_workers_dataloader=0, pin_memory=False)
    dataloaders_ok(
        dm,
...
def test_dm_print_on_setup(data_dir, pi):
    dm = RSVQAHRDataModule(data_dir=data_dir, print_infos=pi, num_workers_dataloader=0, pin_memory=False)
    dm.setup()


def test_dm_shuffle_false(data_dir):
    dm = RSVQAHRDataModule(data_dir=data_dir, shuffle=False, num_workers_dataloader=0, pin_memory=False)
    dm.setup(None)
...
def test_dm_shuffle_none(data_dir):
    dm = RSVQAHRDataModule(data_dir=data_dir, shuffle=None, num_workers_dataloader=0, pin_memory=False)
    dm.setup(None)
    assert not torch.equal(
        next(iter(dm.train_dataloader()))[0],
        next(iter(dm.train_dataloader()))[0],
...
def test_dm_shuffle_true(data_dir):
    dm = RSVQAHRDataModule(data_dir=data_dir, shuffle=True, num_workers_dataloader=0, pin_memory=False)
    dm.setup(None)
    assert not torch.equal(
        next(iter(dm.train_dataloader()))[0],
        next(iter(dm.train_dataloader()))[0],
